...
void loop() {
  ...
  // Complementary filter (accel corrects roll & pitch only)
  roll  = alpha * roll_gyro  + (1.0f - alpha) * acc_roll;
  pitch = alpha * pitch_gyro + (1.0f - alpha) * acc_pitch;
  yaw   = yaw_gyro; // yaw only from gyro (will drift over time!)
...
//how can we make a  3d character/sprite break out of the fixed 3d grid?
//form material requests 
//Result :we want to see a character out of the fixedd grid..dynamic 
